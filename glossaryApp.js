...
const APP_VERSION='3.0.201',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_BOOKMARK_KEY='riyoshi_glossary_today_bookmarks_v1',TEXT_SCALE_KEY='riyoshi_glossary_text_enlarged_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function recordGlobalRoundEvaluation(term,status){if(!term||!states[status])return;const meta=roundMeta(),answers=meta.current.answers;answers[term.id]=status;saveLearning()}
function startNextLap(){const meta=roundMeta(),breakdown=globalCurrentBreakdown();if(breakdown.unlearned!==0)return;if(!confirm(`お疲れ様でした！${lapLabel(meta.completed+1)}が完了しました。\n次の周回（${lapLabel(meta.completed+2)}）を開始しますか？`))return;const completedRound=meta.completed+1,record={round:completedRound,at:dateTime(),safe:breakdown.safe,caution:breakdown.caution,danger:breakdown.danger,unlearned:0,total:breakdown.total};meta.history.push(record);if(!flushLearning()){meta.history.pop();saveLearning();return}let saved=false;try{const persisted=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}')?.[ROUND_META_KEY]?.history||[],item=persisted.find(value=>Number(value.round)===completedRound);saved=Boolean(item&&Number(item.safe)===record.safe&&Number(item.caution)===record.caution&&Number(item.danger)===record.danger&&Number(item.unlearned)===0&&Number(item.total)===record.total)}catch(_){}if(!saved){meta.history.pop();saveLearning();return}meta.completed=completedRound;meta.current={answers:{},correctAnswers:{}};resetCurrentRoundStatuses();flushLearning();renderHome()}
...
function quizProgressSummary(){const s=quizHomeStats(),status=stats(),pct=s.total?Math.round(s.learned/s.total*100):0,isComplete=pct===100,progressBar=isComplete?`<span class="category-row-bar quiz-progress-bar is-complete"><span class="quiz-progress-barber"></span></span>`:`<span class="category-row-bar quiz-progress-bar">${percentSegment('safe',status.safe,status.total||1)}${percentSegment('caution',status.caution,status.total||1)}${percentSegment('danger',status.danger,status.total||1)}${percentSegment('unlearned',status.unlearned,status.total||1)}</span>`;return `<section class="summary-card quiz-progress-card"><button type="button" class="quiz-round-count" onclick="Glossary.openRoundHistory()" aria-label="${lapLabel(roundNumber())}、過去の周回成績を表示">${lapLabel(roundNumber())}</button><div class="ring" style="--progress:${pct}%"><span>${pct}%</span></div><div><div class="summary-label">学習進捗</div><div class="summary-main">${s.learned} / ${s.total}語</div><div class="summary-sub">学習済み ${s.learned}語　未学習 ${s.unlearned}語<br>累計正答率 ${s.answered?s.rate+'％':'未解答'}</div></div>${progressBar}${isComplete?`<div class="lap-complete-banner"><p>${lapLabel(roundNumber())}、全${s.total}語が完了しました。</p><button type="button" class="lap-next-button" onclick="Glossary.startNextLap()">次の周回を始める</button></div>`:''}</section>`}
...
function todayResponse(term=currentTerm()){const question=quizData.questions?.[term.id],signature=question?`${question.question}|${question.correct}|${(question.distractors||[]).join(',')}`:'';const cached=todayAnswers.get(term.id);if(cached&&(cached.completed||cached.signature===signature))return cached;const distractors=question?shuffle([...question.distractors]).slice(0,2):[],options=question?shuffle([{text:question.correct,correct:true},...distractors.map(text=>({text,correct:false}))]):[],response={question:question?.question||'',source:question?.source||'',signature,options,hold:!question,attempts:[],completed:false,autoStatus:'',currentStatus:'',counted:false,roundCounted:false,originalResult:'',finalResult:'',unable:false,unableTiming:'',recorded:false};todayAnswers.set(term.id,response);persistTodaySession();return response}
...
function adjustQuizTotals(term,result,delta){const key=stateKey(term),s=termState(term)||{},correct=result==='first_correct';learning[key]={...s,quizCorrect:Math.max(0,(Number(s.quizCorrect)||0)+(correct?delta:0)),quizWrong:Math.max(0,(Number(s.quizWrong)||0)+(correct?0:delta))};saveLearning()}
